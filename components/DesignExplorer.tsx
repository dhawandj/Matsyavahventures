...
const DesignExplorer: React.FC<DesignExplorerProps> = ({ selectedCategory }) => {
  const filteredProjects = selectedCategory 
    ? PROJECTS.filter(p => p.category === selectedCategory || (selectedCategory === 'Outdoor & Garden' && p.category === 'Outdoor'))
    : PROJECTS.slice(0, 6); // Show latest if nothing selected
      ...
      {filteredProjects.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
          {filteredProjects.map((project) => (
            <div 
              key={project.id} 
              className="group bg-white rounded-3xl overflow-hidden border border-stone-200 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500"
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm p-3 rounded-2xl opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 cursor-pointer shadow-xl">
        ...
        <div className="py-32 text-center bg-stone-50 rounded-[3rem] border-2 border-dashed border-stone-200 mx-auto max-w-4xl">
          <div className="w-16 h-16 bg-stone-100 rounded-full flex items-center justify-center mx-auto mb-6 text-stone-400">
            {/* Fix: LayoutGrid is now imported */}
            <LayoutGrid size={32} />
          </div>
